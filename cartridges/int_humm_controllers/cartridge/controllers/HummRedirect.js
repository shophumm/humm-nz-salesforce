...
function prepare() {
    ...
    if (gatewayURL.indexOf('.' + countryCode) > -1) {
        app.getView({
            orderObject: OrderMgr.getOrder(request.httpParameterMap.orderNo.stringValue),
        ...
        Transaction.commit();
        var redirectURL = URLUtils.https('COSummary-Start', 'error', Resource.msg('humm.gatway.url', 'humm', null) + ' ' + countryCode.toUpperCase());
        app.getView({
